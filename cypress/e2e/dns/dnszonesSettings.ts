...
export const clearTextfieldValue = (dataCy: string) => {
  cy.dataCy(dataCy).clear();
  cy.dataCy(dataCy).should("have.value", "");
};
...
export const clearAndAddAdjustedNumberValue = (
  ...
  cy.dataCy(dataCy).type(`{selectall}${value}`);
  cy.dataCy(dataCy).should("have.value", value);
};

export const setDnsZoneAuthNameserver = (nameserver: string) => {
  clearTextfieldValue("dns-zones-tab-settings-textbox-idnssoamname");
  cy.dataCy("dns-zones-tab-settings-textbox-idnssoamname").type(nameserver);
  checkValueExists("dns-zones-tab-settings-textbox-idnssoamname", nameserver);
};

export const setDnsZoneAdminEmail = (email: string) => {
  clearTextfieldValue("dns-zones-tab-settings-textbox-idnssoarname");
  cy.dataCy("dns-zones-tab-settings-textbox-idnssoarname").type(email);
...
export const setDnsZoneDynamicUpdate = (isDynamicUpdateChecked: boolean) => {
  clearTextfieldValue("dns-zones-tab-settings-checkbox-idnsallowdynupdate");
...
export const setDnsZoneBindUpdatePolicy = (bindUpdatePolicy: string) => {
  clearTextfieldValue("dns-zones-tab-settings-textbox-idnsupdatepolicy");
  cy.dataCy("dns-zones-tab-settings-textbox-idnsupdatepolicy").type(
    bindUpdatePolicy
  );
  checkValueExists(
...
export const setDnsZoneNSecParamRecord = (nsecParamRecord: string) => {
  clearTextfieldValue("dns-zones-tab-settings-textbox-nsec3paramrecord");
  cy.dataCy("dns-zones-tab-settings-textbox-nsec3paramrecord").type(
    nsecParamRecord
  );
  checkValueExists(
...
export const setDnsZoneDefaultValues = () => {
  setDnsZoneAuthNameserver("server.ipa.demo.");
  setDnsZoneAdminEmail("admin@example.com");
  setDnsZoneSoaRefresh(3604);
  setDnsZoneSoaRetry(900);
  setDnsZoneSoaExpire(1209600);
  setDnsZoneSoaMinimum(3600);
  setDnsZoneDefaultTtl(0);
  setDnsZoneTtl(0);
  setDnsZoneDynamicUpdate(false);
  setDnsZoneBindUpdatePolicy(
    "grant DOM-IPA.DEMO krb5-self * A; grant DOM-IPA.DEMO krb5-self * AAAA; grant DOM-IPA.DEMO krb5-self * SSHFP;"
  );
  addDnsZoneAllowQuery("any");
  addDnsZoneAllowTransfer("none");
  removeDnsZoneForwarder("192.168.1.0/24");
  removeDnsZoneForwarder("10.0.0.0/8");
  selectDnsZoneForwardPolicyRadio("first");
  allowDnsZoneSyncPtr(true);
  allowDnsZoneInLineDnsSecSigning(false);
  setDnsZoneNSecParamRecord("");
};

Given("I am on DNS zone {string} > Settings page", (dnsZoneName: string) => {

...
export const addPermissionToDnsZone = (dnsZoneName: string) => {
  ...
  cy.dataCy("dns-zones-tab-settings-kebab-add-permission").click();
  cy.dataCy("dns-zones-add-remove-permission-modal").should("be.visible");

  cy.dataCy("dns-zones-add-remove-permission-modal-button-ok").click();
...
export const removePermissionFromDnsZone = (dnsZoneName: string) => {
  ...
  cy.dataCy("dns-zones-tab-settings-kebab-remove-permission").click();
  cy.dataCy("dns-zones-add-remove-permission-modal").should("be.visible");
...
Given(
  "DNS zone {string} exists and has no permission",
  (dnsZoneName: string) => {
    navigateTo("dns-zones");
    ...
    entryExists(parseZoneName(dnsZoneName));
    navigateTo("dns-zones/" + dnsZoneName);
    removePermissionFromDnsZone(dnsZoneName);
  }
);

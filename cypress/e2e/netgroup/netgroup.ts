...
const elementTypeToTabDataCyMap: Record<string, string> = {
  user: "netgroups-tab-settings-tab-users",
  group: "netgroups-tab-settings-tab-groups",
  host: "netgroups-tab-settings-tab-hosts",
  hostgroup: "netgroups-tab-settings-tab-hostgroups",
  externalhost: "netgroups-tab-settings-tab-externalhosts",
};

const elementTypeToTabDataCy = (elementType: string): string => {
  const key = elementType.toLowerCase();
  const value = elementTypeToTabDataCyMap[key];
  return value;
};

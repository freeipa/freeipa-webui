...
const IdRanges = () => {
  ...
  const isElementSelectable = () => true;
  const selectableElementsTable = idRanges.filter(isIdRangeSelectable);
  const setIdRangesSelected = (element: IdRange, isSelecting?: boolean) => {
    setSelectedElements((prev) => {
      const exists = prev.find((e) => e.cn === element.cn);
      if (isSelecting) {
        return exists ? prev : [...prev, element];
      } else {
        return prev.filter((e) => e.cn !== element.cn);
      }
    });
  };
  ...
  const bulkSelectorData = {
    ...
    selectableTable: selectableElementsTable,
    nameAttr: "cn",
  };

...
interface RulesData {
  ...
  selectableRulesTable: SudoRule[];
  setRulesSelected: (rule: SudoRule, isSelecting?: boolean) => void;
...
const SudoRulesTable = (props: PropsToTable) => {
  ...
  const isRuleSelected = (rule: SudoRule) => {
    if (
  ...
  useEffect(() => {
    if (props.rulesData.selectedRules.length > 0) {
      props.buttonsData.updateIsDeleteButtonDisabled(false);
      const selectedRules: SudoRule[] = props.rulesData.selectedRules;
      if (
        ...
        if (selectedRules.length > 0) {
          const equalStatus = checkEqualStatusSudoRule(
  ...
  const header = (
    <Tr>
      <Th modifier="wrap" screenReaderText="Select rows"></Th>

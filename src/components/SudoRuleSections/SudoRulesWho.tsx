...
import IpaDualRadios from "src/components/Form/IpaDualRadios";
// RPC
import {
...
const SudoRulesWho = (props: PropsToSudoRulesWho) => {
  ...
  //  - If 'specify' radio is selected (just modified) and new users should be added:
  ...
  // - Instead of using the name of the buttons, it will be referred as "first" as per the 'IpaDualRadios' component
  const [optionChecked, setOptionChecked] = React.useState("first");

  // - When 'usercategory' is "all", disable checkboxes
  const anyoneRadioSelected = optionChecked === "first";

  // - Modify the 'setIsAnyoneSelected' function to set the value of 'optionChecked'
  React.useEffect(() => {
    props.setIsAnyoneSelected(anyoneRadioSelected);
  }, [anyoneRadioSelected]);
    ...
    <Flex>
      ...
      <FlexItem>
        <IpaDualRadios
          ...
          name="usercategory"
          firstRadioName="Anyone"
          secondRadioName="Specified Users and Groups"
      ...
      >
        ...
        >
          <KeytabTableWithFilter
            ...
            checkboxesDisabled={anyoneRadioSelected}

...
interface RulesData {
  ...
  updateSelectedRules: (rule: SudoRule[], isSelected: boolean) => void;
}
...
const BulkSelectorSudoRulesPrep = (props: PropsToBulkSelectorPrep) => {
  ...
  const selectableElementsPage = props.shownElementsList.filter(
    props.elementData.isSudoRuleSelectable
  );
  ...
  const group_id_list = props.elementData.selectedRules.map((rule) => {
    return rule.cn;
  });
...
export default BulkSelectorSudoRulesPrep;

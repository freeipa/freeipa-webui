...
interface PropsToAddRule {
  ...
  onRefresh?: () => void;
  elementsInTable?: string[]; // To filter available elements
}
...
const AddRule = (props: PropsToAddRule) => {
  ...
  const [groupsAvailable, setGroupsAvailable] = React.useState<string[]>([]);
  ...
  const groupsQuery = useGetGenericListQuery("group");
  ...
  // If 'elementsInTable' is provided/updated, update the available groups
  React.useEffect(() => {
    if (
      props.elementsInTable !== undefined &&
      props.elementsInTable.length > 0 &&
      groupsAvailable.length > 0
    ) {
      const elementsInTable = props.elementsInTable;
      const filteredGroups = groupsAvailable.filter(
        (element) => !elementsInTable.includes(element)
      );
      setGroupsAvailable(filteredGroups);
    }
  }, [props.elementsInTable]);
  ...
  const fields = [
    {
      ...
      pfComponent: (
        ...
        >
          {groupsAvailable.map((option, index) => (

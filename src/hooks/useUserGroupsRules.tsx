...
const useUserGroupsRulesData = (): UserGroupsRulesData => {
  ...
  React.useEffect(() => {
    if (automembersList.length > 0 && !automembersQuery.isFetching) {
      const automembersDataList: AutomemberEntry[] = [];
      // Compare automembers and user groups to obtain the 'description' from a given user group
      automembersList.map((automember) => {
        const group = userGroupsGeneralData.find(
          (group) => group.cn.toString() === automember
        );
        automembersDataList.push({
          automemberRule: automember,
          description:
            (group?.description && group.description.toString()) || "",
        });
      });
      setAutomemberIdsList(automembersDataList);
    }

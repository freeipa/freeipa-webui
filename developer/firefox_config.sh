...
then
    ...
    flatpack_app="org.mozilla.firefox"
    quiet flatpak info "${flatpack_app}" || die "Cannot find Firefox executable"
    if ! grep -q "filesystems=home;" <(flatpak override -u --show org.mozilla.firefox ||:)
    then
        flatpack_error "${flatpack_app}"
    fi

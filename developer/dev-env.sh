...
check_command() {
    [[ -n "$1" ]] ||  die "Internal error: (check_command) commannd not provided"
    quiet command -v "${1}"
}
...
stop_container() {
    ...
    then
        log -e "Failed to remove container."
    else
...
start_scenario() {
    ...
    if grep -Fq "^Erro" <(podman-compose -f "${SCRIPT_DIR}/scenarios/${scenario}/compose.yml" up -d "${extra[@]}" 2>&1)
    then
        die "Unknown error starting container."
    else
...
stop_scenario() {
    ...
    # shellcheck disable=SC2312
    if grep -iF "^error" < <(podman-compose -f "${SCRIPT_DIR}/scenarios/${scenario}/compose.yml" down)
    then
        die -e "Errors found while shutting down ${scenario}."
    fi
...
build_scenario() {
    ...
    check_command ansible-playbook \
        || die "Command 'ansible-playbook' is required for buiilding scenarios"
    if ! check_command podman-compose
    then
        [[ "${scenario}" == "single-server" ]] \
            || die "Command 'podman-compose' is required for buiilding scenarios"
        build_container
        return $?
    fi
...
case "${action}" in
    ...
    "start")
        ...
        ;;
    "kill") [[ -n "${running:-${scenario:-"single-server"}}" ]] && stop_scenario "${running:-${scenario:-"single-server"}}" ;;
    "restart")

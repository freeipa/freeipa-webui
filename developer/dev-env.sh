...
build_container() {
    ...
    # Save image locally
    log "=== Save container image as '${image_name}:latest' ==="
...
is_webserver_running() {
    ...
    [ -f "${dist_dir}/index.html" ] && \
        quiet podman exec webui nc -znr 127.0.0.1 5173
}
...
while getopts ":hB:bcCdfkprs" option "$@"
do
    case "${option}" in
        "h") usage && exit ;;
...
case "${action}" in
    ...
    "cypress"*)
        ...
        podman unshare --rootless-netns npx ${action/:/ } "${extra[@]}"
        ;;

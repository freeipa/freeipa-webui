...
This instructions assume that [`podman`](https://podman.io) is available on the system. Installation of `podman` is not in the scope of this document, but the alternative method with a Python virtual environment can be used:
...
ansible-galaxy collection install developer/requirements.yml
```
...
Instead of the development server, you can bulid the project for production using either
...
As the containers are executing as rootless containers, in its own network namespace, it is needed to use `podman unshare --rootless-netns` to start a browser that is able to access the container network.

With **Firefox**, one can use a separate profile, that can trust the IPA CA certificate. To ease the process a script is provided:

```
./developer/open-firefox.sh https://webui.ipa.test/ipa/modern-ui
```

With **Chrome**, start the browser and manually accept the IPA CA certificate:

```
podman unshare --rootless-netns google-chrome --no-sandbox https://webui.ipa.test/ipa/modern-ui
```

> The `--no-sandbox` option is required for Google as it runs as `root` inside the container.

The default credentials for the development environment are **admin** and **SomeADMINpassword**.
...
> Note: The integration tests, currently, do not cleanup the test environment, it is recommended that you restart the container (`develop/dev-env.sh -r`) before running the integration tests.

Run the integration tests in headless mode with:

```
./develop/dev-env.sh -c
```

You can provide arguments to Cypress using the _long options_. For example, you can run only the hosts tests with:

```
./develop/dev-env.sh -c --spec cypress/e2e/hosts/hosts.feature
```
